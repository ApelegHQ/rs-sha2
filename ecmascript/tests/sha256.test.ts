...
const skip = !sha2.sha256;
const getInstance = () => sha2.sha256();

const shortMsgVectors = loadVectors('SHA256ShortMsg.rsp');
const longMsgVectors = loadVectors('SHA256LongMsg.rsp');

testsFactory('SHA-256', getInstance, 'NIST ShortMsg', shortMsgVectors, skip);
testsFactory('SHA-256', getInstance, 'NIST LongMsg', longMsgVectors, skip);
